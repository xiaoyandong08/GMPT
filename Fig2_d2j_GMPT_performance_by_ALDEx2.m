function Fig2_d2j_GMPT_performance_by_ALDEx2
...
subplot(3,1,2)
hold on;box on;
min_neg_value = floor(min(case_control_data(:,2)));
max_pos_value = max(effect_threshold,ceil(max(case_control_data(:,2))));
% rectangle('Position',[0 0.5 N-1+0.5 max_pos_value-0.5],'EdgeColor', 'n', 'FaceColor', [hex2rgb('F74461') 0.3])
% rectangle('Position',[0 min_neg_value N-1+0.5 -min_neg_value-0.5],'EdgeColor', 'n', 'FaceColor', [hex2rgb('0072BD') 0.3])
plot([0 N-1+0.5],[effect_threshold effect_threshold],'k--')
plot([0 N-1+0.5],[-effect_threshold -effect_threshold],'k--')
...
ylabel('Effect');
xlabel('Species (in descending order of absolute values of effect)')
xlim([0 N-1+0.5])
% ylim([min_neg_value max_pos_value])
set(gca,'fontsize',14,'TickDir','out')
...
subplot(3,1,3)
hold on;box on;
min_neg_value = floor(min(TwoParts_data(:,2)));
max_pos_value = max(effect_threshold,ceil(max(TwoParts_data(:,2))));
% rectangle('Position',[0 0.5 N-1+0.5 max_pos_value-0.5],'EdgeColor', 'n', 'FaceColor', [hex2rgb('F74461') 0.3])
% rectangle('Position',[0 min_neg_value N-1+0.5 -min_neg_value-0.5],'EdgeColor', 'n', 'FaceColor', [hex2rgb('0072BD') 0.3])
plot([0 N-1+0.5],[effect_threshold effect_threshold],'k--')
plot([0 N-1+0.5],[-effect_threshold -effect_threshold],'k--')
...
save('N30_ALDEx2_GMPT.txt','GMPT_data','-ascii');
save('N30_ALDEx2_CaseControl.txt','case_control_data','-ascii');
save('N30_ALDEx2_TwoParts.txt','TwoParts_data','-ascii');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


Binary = readtable('N30_Binary_classification_metrics_by_ALDEx2.txt');

methods = {'GT','CC','TP'};

for m = 1 : length(methods)
        
    index = find(strcmp(Binary.Group,methods{m}));
    if length(index)~=N-1
        aa = 1;
    end
