function Many_times_different_sample_size_v2(dynamical_parameter,N,C,delta,diag,VarianceType,exchange_rate,Num_sample,simulation,noise,shuffle_SF_Amatrix)
% N = [100];
% C = [0.4];
% delta = [0.2];
% diag = -1;
...
for i = 1 : simulation
    ...
    [A{i},r{i},OTU_table{i},OTU_table_Cdiff{i},relative_OTU_table{i},relative_OTU_table_Cdiff{i},abundance_Cdiff{i},abundance_error_Cdiff{i}]=...
        ...
        Cdiff_disease_abundance,Cdiff_health_abundance,Strongest_inhibitor_present,select_white_black_mixed,noise,shuffle_SF_Amatrix);
end
...
while 1
    ...
    [A,r] = Generate_Network_A_of_BandW(N,C,delta,diag,VarianceType,time,FunctionType,h1,h2,Cdiff,abundance_type,Cdiff_disease_abundance,Cdiff_health_abundance,select_white_black_mixed,shuffle_SF_Amatrix);
    %%% generate B, W
...
simulation_relative_OTU_table_Cdiff = relative_OTU_table_Cdiff;
simulation_absolute_OTU_table_Cdiff = OTU_table_Cdiff;
...
folder = ['./' father_folder '/relative' '_With_Cdiff_exchange_' exchange_name '/N' num2str(N) 'S' num2str(simulation)];
Generate_file_for_LefSe_for_TWO_parts(folder,simulation_relative_OTU_table_Cdiff,N,{'first','second'},Num_sample);
